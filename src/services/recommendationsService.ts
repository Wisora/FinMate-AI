...
const INITIAL_RECOMMENDATIONS: Recommendation[] = [
  {
    ...
    category: 'savings',
    description: 'Set up automatic recurring transfer to your 3-Month Emergency Fund goal every 1st of the month to build reserves effortlessy.',
    impact: 'high',
  ...
  },
  {
    ...
    category: 'debt',
    description: 'Redirect $150/mo from discretionary dining to your 18.5% credit card debt. You will save an estimated $420 in interest charges.',
    impact: 'high',
  ...
  },
  {
    ...
    category: 'budgeting',
    description: 'Food & Dining expenses represent 22% of monthly spending. Preparing 2 extra home meals per week will save ~$210/mo.',
    impact: 'medium',
  ...
  },
  {
    ...
    description: 'Split investment contributions into bi-weekly $250 index fund buys to smooth market volatility and lower long-term risk.',
    impact: 'high',
...
export const getRecommendations = (): Recommendation[] => {
  ...
  try {
    localStorage.setItem(RECS_STORAGE_KEY, JSON.stringify(INITIAL_RECOMMENDATIONS));
  } catch (e) {
...
export const toggleApplyRecommendation = (id: string): Recommendation[] => {
  ...
  const updated = current.map((r) => (r.id === id ? { ...r, applied: !r.applied } : r));
  try {
