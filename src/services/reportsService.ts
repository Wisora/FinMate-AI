...
const SAMPLE_TRANSACTIONS: Transaction[] = [
  { id: 't1', title: 'Tech Monthly Salary', amount: 5400, type: 'income', category: 'Salary', date: '2026-07-01', isRecurring: true },
  { id: 't2', title: 'Freelance Design Retainer', amount: 850, type: 'income', category: 'Freelance', date: '2026-07-12' },
  { id: 't3', title: 'Apartment Rent & Water', amount: 1850, type: 'expense', category: 'Housing', date: '2026-07-02', isRecurring: true },
  { id: 't4', title: 'Whole Foods Grocery', amount: 480, type: 'expense', category: 'Food & Dining', date: '2026-07-05' },
  { id: 't5', title: 'Dining Out & Bistro', amount: 320, type: 'expense', category: 'Food & Dining', date: '2026-07-15' },
  { id: 't6', title: 'Electric & High-Speed Fiber', amount: 165, type: 'expense', category: 'Utilities', date: '2026-07-08', isRecurring: true },
  { id: 't7', title: 'Uber & EV Charging', amount: 190, type: 'expense', category: 'Transport', date: '2026-07-18' },
  { id: 't8', title: 'Streaming & Cinema Pass', amount: 85, type: 'expense', category: 'Entertainment', date: '2026-07-10' },
  { id: 't9', title: 'Health Insurance & Gym', amount: 240, type: 'expense', category: 'Health', date: '2026-07-03' },
  { id: 't10', title: 'Gadgets & Summer Apparel', amount: 290, type: 'expense', category: 'Shopping', date: '2026-07-22' },
];
...
export const getFinancialSummary = (): FinancialSummary => {
  ...
  const totalDebtCurrent = debtGoals.reduce((sum, g) => sum + g.currentAmount, 0);
  const debtProgress = totalDebtTarget > 0 ? Math.round((totalDebtCurrent / totalDebtTarget) * 100) : 0;

  const investmentGoals = goals.filter((g) => g.category === 'investment');
  const investments = investmentGoals.reduce((sum, g) => sum + g.currentAmount, 0);
  ...
  const healthScore = Math.min(98, Math.max(45, savingsScore + debtScore + investmentScore + 20));
...
export const getCategoryBreakdown = () => {
  ...
  return Object.keys(totals).map((cat) => ({
    category: cat,
    amount: totals[cat],
    percentage: totalExpense > 0 ? Math.round((totals[cat] / totalExpense) * 100) : 0,
    color: colors[cat] || '#9CA3AF',
  }));
};

export const fetchAISummaryReport = async (timeframe: string, language: string, currency: string): Promise<string> => {
