...
import { Sparkles, Check, ArrowUpRight, DollarSign, ShieldAlert, Zap } from 'lucide-react';
    ...
    <div className="flex flex-col justify-between p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-200">
      <div>
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full border ${impactBadges[recommendation.impact]}`}>
            {recommendation.impact.toUpperCase()} IMPACT
          </span>
        </div>

        <h4 className="font-bold text-slate-900 dark:text-white text-base mb-1.5">{recommendation.title}</h4>
        <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-4">{recommendation.description}</p>
        ...
        {recommendation.potentialSavings > 0 && (
          <div className="mb-4 p-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/40 flex items-center justify-between">
            <span className="text-xs font-medium text-emerald-800 dark:text-emerald-300 flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Est. Annual Benefit:
            </span>
