...
export const AssistantChat: React.FC<AssistantChatProps> = ({ financialSummary }) => {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('finmate_chat_history', JSON.stringify(messages.slice(-20)));
    } catch (e) {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      const result = await sendChatMessage(messageText, messages, financialSummary, language, currency);
      ...
      <div className="flex items-center justify-between px-5 py-4 bg-slate-900 text-white border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Contextual financial guidance in {language.toUpperCase()}</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-900/40">
        ...
        {loading && (
          <div className="flex gap-3 max-w-[80%] mr-auto items-center text-slate-500 dark:text-slate-400 text-xs p-3">
            ...
            <span>FinMate AI is generating personalized financial advice...</span>
          </div>
        )}
