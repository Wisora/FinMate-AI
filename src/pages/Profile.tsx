...
export const Profile: React.FC<ProfileProps> = ({ user, onUpdateProfile, onNavigate, showToast }) => {
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
          Manage your personal account, financial persona, and active membership subscription.
        </p>
      </div>
      ...
      <div className="p-8 rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
        ...
        {/* Edit Form */}
        {isEditing ? (
          <form onSubmit={handleSave} className="space-y-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none"
              >
                <option value="Aggressive Debt Payoff & Saver">Aggressive Debt Payoff & Saver</option>
                <option value="Index Fund DCA Investor">Index Fund DCA Investor</option>
                <option value="Balanced Budget Enthusiast">Balanced Budget Enthusiast</option>
                <option value="Emergency Reserve Builder">Emergency Reserve Builder</option>
              </select>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-base text-white">Subscription Plan Status</h3>
              </div>
              <p className="text-xs text-slate-300 mt-0.5">
                Current tier: <strong className="text-emerald-400 capitalize">{user.plan} Tier</strong> (Joined: {user.joinedDate})
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
            <span className="text-xs text-slate-500 dark:text-slate-400">Net Surplus</span>
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
            <span className="text-xs text-slate-500 dark:text-slate-400">Savings Target Pct</span>
            <div className="text-xl font-bold text-slate-900 dark:text-white mt-1">
