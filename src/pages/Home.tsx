    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-slate-900 text-white p-8 sm:p-12 shadow-xl">
        ...
        <div className="relative z-10 max-w-3xl space-y-6">
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight leading-tight text-white">
            Take Control of Your Savings, Debt & Investments with <span className="text-emerald-400">FinMate AI</span>
          </h1>

          <p className="text-slate-300 text-base sm:text-lg leading-relaxed">
            Welcome back, <strong className="text-white">{user.name}</strong>. FinMate combines real-time cashflow analytics, proactive overspending alerts, and conversational Gemini 3.6 AI recommendations to fast-track your wealth goals.
          </p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Tri-Tier Goal Engine</h3>
          <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Track savings buffers, debt payoff snowballs, and index investment portfolios with target dates, milestones, and automated progress updates.
          </p>
          <button
            onClick={() => onNavigate('dashboard')}
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Gemini 3.6 Conversational Coach</h3>
          <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Ask natural queries like *"How much did I save last month?"* or *"Generate a July expense report"* with context-aware financial answers.
          </p>
          <button
            onClick={() => onNavigate('dashboard')}
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Multilingual & Global Currencies</h3>
          <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Instant support for English, Afrikaans, French, Spanish, and Arabic with RTL layout formatting, alongside USD, EUR, ZAR, GBP, and JPY currencies.
          </p>
          <button
      ...
      <section className="p-8 rounded-3xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
          <div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Why FinMate AI?</h2>
            <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">
              Built for proactive money management, accessibility, and high financial health scores.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            ...
            <h4 className="font-bold text-sm text-slate-900 dark:text-white">Proactive Alerts</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Get notified when expenses trend higher than income or goals hit 90% completion.</p>
          </div>

          <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            <Award className="w-5 h-5 text-emerald-500 mb-2" />
            <h4 className="font-bold text-sm text-slate-900 dark:text-white">Health Score Gauge</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Automated 0-100 score analyzing savings ratio, debt-to-income, and investments.</p>
          </div>

          <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            <TrendingUp className="w-5 h-5 text-emerald-500 mb-2" />
            <h4 className="font-bold text-sm text-slate-900 dark:text-white">Smart Insights</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Tailored advice on meal budgets, high-yield funds, and credit payoff strategies.</p>
          </div>

          <div className="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            <ShieldAlert className="w-5 h-5 text-emerald-500 mb-2" />
            <h4 className="font-bold text-sm text-slate-900 dark:text-white">Local Persistence</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">100% offline-friendly state persistence in browser localStorage for privacy.</p>
          </div>
        </div>
      </section>
    </div>
